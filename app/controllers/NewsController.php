...
class NewsController extends Controller
{
     ...
     */
    public function details($slugOrId)
    {
        // Ưu tiên tìm theo slug
        $post = $this->newsModel->findBySlug($slugOrId);
        // Nếu không có thì thử tìm theo id
        if (!$post && is_numeric($slugOrId)) {
            $post = $this->newsModel->find($slugOrId);
        }
        if (!$post) {
            $post = null;
        }
        include __DIR__ . '/../views/news/details.php';
    }

    // --- Helper function thay thế jsonResponse trait ---
